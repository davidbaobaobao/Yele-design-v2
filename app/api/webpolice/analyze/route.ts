...
const YELE_TEXT: Record<Locale, { effortLabel: string; effortFlavor: string; summary: string; rant: string; sells: string; says: string; hears: string; personality: string; designYear: number; designYearWhy: string }> = {
  en: { effortLabel: 'Handcrafted by Yele themselves', effortFlavor: 'The suspects ARE the police. Case dismissed with a wink.', summary: 'The only website to ever make the Web Police blush. 105/100, no notes — get a room.', rant: "Look, we tried to find something. We really did. We dusted the whole site for the usual crimes — the purple gradients, the rounded-card soup, the stock photos of people high-fiving a robot — and came back with nothing but fingerprints of actual taste.\n\nEvery pixel looks like a decision, not an accident. The typography has opinions, the spacing can breathe, and nobody bolted on a fake dashboard to look busy. Frankly it's showing off. 105/100, the extra five points are for making the rest of the internet look bad.", sells: "Beautiful websites for small businesses — and yes, I could tell in about half a second. That's kind of the whole point.", says: 'We build beautiful websites.', hears: 'Oh. They actually meant it.', personality: 'The kid who did the homework AND made it look effortless.', designYear: 2026, designYearWhy: 'It looks like next year, honestly.' },
  es: { effortLabel: 'Hecha a mano por el propio Yele', effortFlavor: 'Los sospechosos SON la policía. Caso cerrado con un guiño.', summary: 'La única web que ha hecho sonrojar a la Policía Web. 105/100, sin objeciones — buscaos un cuarto.', rant: 'Mira, lo intentamos. De verdad. Peinamos toda la web buscando los delitos de siempre — los degradados morados, la sopa de tarjetas redondeadas, las fotos de stock de gente chocando los cinco con un robot — y solo encontramos huellas de buen gusto.\n\nCada píxel parece una decisión, no un accidente. La tipografía tiene criterio, el espaciado respira y nadie ha pegado un dashboard falso para parecer ocupado. Sinceramente, está presumiendo. 105/100; los cinco puntos extra son por dejar en evidencia al resto de internet.', sells: 'Webs bonitas para pequeños negocios — y sí, lo pillé en medio segundo. De eso va justamente la cosa.', says: 'Hacemos webs bonitas.', hears: 'Ah. Va en serio.', personality: 'El empollón que hizo los deberes Y encima queda de guay.', designYear: 2026, designYearWhy: 'Parece del año que viene, la verdad.' },
  zh: { effortLabel: 'Yele 自己做的', effortFlavor: '嫌疑人就是警察本人。眨个眼，结案。', summary: '唯一一个让网页警察脸红的网站。105/100，挑不出毛病，我们决定给它送面锦旗。', rant: '说真的，我们是想挑毛病的。整个站从上到下排查了一遍：紫色渐变、圆角卡片堆成汤、西装大哥握手的图库照片 —— 一个都没抓到，只找到了「有品味」的指纹。\n\n每个像素都像是有人认真决定过的，不是随手摆的。字体有主见，间距能喘气，也没硬塞一个假仪表盘来装忙。说白了就是在炫技。105/100，多出来的五分，是奖励它把互联网上其他网站衬托得那么惨。', sells: '给小生意做好看的网站 —— 而且我半秒就看出来了。这不正是重点嘛。', says: '我们做好看的网站。', hears: '哦，人家是玩真的。', personality: '那个既写完了作业、还顺手做得毫不费力的学霸。', designYear: 2026, designYearWhy: '说实话，它看着像明年才该有的设计。' },
}
...
- "sells": a funny 1-2 sentence GUESS of what this website is selling or offering — but guess ONLY from what you can literally SEE: the big hero headline, the buttons/CTAs, and the images. Do NOT use the domain name, meta text, or outside knowledge of the brand. If the hero is vague/abstract/buzzwordy, commit to a confident but hilariously WRONG guess (that wrong guess IS the point — it proves the messaging is unclear). If it's crystal clear, nail it plainly (and it can be a bit smug about how easy it was).
...
async function visionAnalyze(images: { data: string; mime: string }[], hints: string[], languageName: string, roastStyle: string, serious: boolean): Promise<VisionResult> {
  ...
  const toneText = serious
    ? `\n\nWrite every "reason" field, the "summary" and the "rant" in ${languageName}.\n\nSERIOUS-MODE OVERRIDE (this takes precedence over any "Web Police", "roast", "funny", "savage" or "playful" instruction above): Write as a professional, constructive web-design consultant giving honest, plain-language feedback a small-business owner can act on. NO jokes, NO slang, NO insults, NO emojis, NO puns, NO persona. "summary" = one clear, professional one-sentence verdict on the design. "rant" = 1–2 short paragraphs of constructive assessment: what works, what to improve, and whether the site's message is clear. "sells" = a plain, accurate one-sentence description of what the site appears to offer (do NOT make a deliberately wrong guess; if it is genuinely unclear, say so plainly). "saysHears": "says" = the site's main claim or call-to-action; "hears" = what a typical visitor actually understands (clear and factual, not a joke). "personality" = an empty string. CRITICAL — SCORING IS IDENTICAL TO FUN MODE: score EXACTLY as strictly as the bands above. The professional, polite tone must NOT make you more generous or inflate any number. Apply the exact same harsh standards (a generic/template site is still 35–50, obvious stock photos still force imagery to 20–35, "inoffensive but generic" is NOT a 60). Only the WORDING of the explanations changes in serious mode — the scores must be the same as they would be in fun mode for the same screenshot.`
    ...
    try {
      ...
      return {
        ...
        data: {
          ...
          sells: String(parsed.sells || '').slice(0, 240) || undefined,
