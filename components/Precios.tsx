  ...
  return (
    <motion.div
      ...
      className={`relative flex flex-col rounded-3xl p-8 cursor-default ${
        plan.highlighted
          ? 'bg-ink text-white shadow-[0_24px_64px_rgba(0,0,0,0.2)] ring-1 ring-white/10'
          : withShadow
            ? 'bg-base text-ink shadow-[0_16px_56px_rgba(0,0,0,0.14)] ring-1 ring-black/[0.07]'
            : 'bg-base text-ink'
      }`}
    >
      {/* Spotlight overlay */}
      ...
      {plan.badge && (
        <span className={`absolute -top-3.5 left-1/2 -translate-x-1/2 text-xs font-body font-semibold px-3 py-1 rounded-full ${
      ...
      <div className="relative mb-6">
        ...
        <div className="flex items-end gap-1 mb-1">
          <motion.span
            ...
            className="font-display font-semibold text-5xl tracking-tight"
          >
            {price}
          </motion.span>
          <span className={`font-body text-sm mb-2 ${plan.highlighted ? 'text-white/50' : 'text-muted'}`}>{isAnnual ? '/año' : '/mes'}</span>
        </div>
      </div>
        ...
        {t(features.join('||'), plan.en.features.join('||'))
          ...
          .map((feat: string) => (
            <li key={feat} className="flex items-start gap-2.5">
              ...
              <span className={`font-body text-sm ${plan.highlighted ? 'text-white/80' : 'text-ink'}`}>
                {feat}
              </span>
            </li>
          ))}
  ...
  return (
    <section id="precios" className={`py-14 md:py-20 ${noBg ? '' : 'bg-white'}`}>
      <div className="max-w-6xl mx-auto px-6">
        ...
        >
          <h2 className="font-display font-semibold text-5xl md:text-6xl text-ink tracking-tight mb-6">
            {t('Precios', 'Pricing')}
          </h2>

          {/* Toggle — hidden in single-plan mode */}
          {!singlePlan && <div className="inline-flex items-center bg-base rounded-xl p-1 gap-1">
            <button
              onClick={() => setBilling('monthly')}
              className={`relative font-body text-sm px-5 py-2 rounded-lg transition-colors duration-200 cursor-pointer ${
            ...
            <button
              ...
              className={`relative flex items-center gap-1.5 font-body text-sm px-5 py-2 rounded-lg transition-colors duration-200 cursor-pointer ${
        ...
        <p className="text-center font-body text-sm text-muted mt-3">
          {t(
          ...
          <a href="#contacto" className="text-ink font-medium underline underline-offset-2 hover:text-[#0066CC] transition-colors">
